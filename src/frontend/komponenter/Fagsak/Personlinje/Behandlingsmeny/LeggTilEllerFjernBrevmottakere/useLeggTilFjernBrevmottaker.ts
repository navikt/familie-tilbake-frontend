...
const useLeggTilFjernBrevmottaker = () => {
    ...
    const { behandling, settBehandling } = useBehandling();
    const { behandlingId } = useSakOgBehandlingParams();
    const { request } = useHttp();

    const åpenBehandling =
        behandling?.status !== RessursStatus.SUKSESS ? undefined : hentDataFraRessurs(behandling);
            ...
            onSubmit(
                {
                    ...
                    url: `/familie-tilbake/api/brevmottaker/${behandlingId}`,
                },
    ...
    const fjernMottaker = (mottakerId: number) => {
        return request<void, IBehandling>({
            method: 'DELETE',
            url: `/familie-tilbake/api/brevmottaker/${behandlingId}/${mottakerId}`,
