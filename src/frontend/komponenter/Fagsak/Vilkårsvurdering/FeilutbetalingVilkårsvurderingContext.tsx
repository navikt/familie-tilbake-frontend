...
const [
    ...
    const [feilutbetalingVilkårsvurdering, settFeilutbetalingVilkårsvurdering] = React.useState<
    ...
    React.useEffect(() => {
        if (skjemaData) {
            ...
            const behandletPerioder = skjemaData.filter(per => !per.foreldet && erBehandlet(per));
    ...
    const harEndretOpplysninger = (ikkeforeldetPerioder: VilkårsvurderingPeriodeSkjemaData[]) => {
        if (feilutbetalingVilkårsvurdering?.status === RessursStatus.SUKSESS) {
            ...
            return ikkeforeldetPerioder.some(skjemaPeriode => {
                ...
                const endretGodTro = !deepEqual(vurderingSkjema?.godTro, vurderingPeriode?.godTro);
